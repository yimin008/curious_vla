...
def main(cfg: DictConfig) -> None:
    ...
    # Keep extra columns required to reproduce pseudo closed-loop aggregation offline.
    # `weight` is produced by SceneAggregator (Gaussian kernel over endpoint/startpoint distance).
    extra_cols = [
        c
        for c in [
            "weight",
    ...
    pdm_score_df = pdm_score_df[["token", "valid"] + extra_cols + score_cols]
